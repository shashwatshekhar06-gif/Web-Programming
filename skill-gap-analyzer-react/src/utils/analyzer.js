...
import { SKILLS_DATABASE, RESOURCES_DATABASE, MOTIVATIONS } from '../data/skillsDatabase';
...
class SkillAnalyzer {
  ...
  parseJobDescription(text) {
    ...
    // Find end of nice-to-have section
  ...
  analyze() {
    ...
    // deepen: Subset of matched that need improvement
  ...
  _calculateJobReadyDate(missingCount, deepenCount) {
    const hoursPerWeek = parseInt(this.userProfile.studyHours) || 10;
    const totalEffort = missingCount * 15 + deepenCount * 8;
    const weeksNeeded = Math.max(8, Math.ceil(totalEffort / hoursPerWeek));
    const cappedWeeks = Math.min(weeksNeeded, 16);
